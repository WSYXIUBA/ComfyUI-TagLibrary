...
`0others` / `trio` / `pair` / `group of girls` 这类自造词模型基本读不懂，单人锁挡住的
那批在**关掉单人锁的群像场景**直接表现为人数失控。换成了有贴数的真标签：
...
## v1.13.1 — 单人锁 / 简背景 / 人数锚点 / 未成年锁 / NL 若干调整（2026-09-23）
...
- 下拉、预设管理弹窗、挑选器预设页都不再分「内置 / 我的」两组 —— 就是一串预设
  （自存的排前面；内置的 13 条场景预设照旧保留，一条没删）。
- 预设行高亮（1.12.1 加的「选了就常亮」）改成：只有**面板内容跟这条预设一致**时才亮。
  改词、清空、🎲 填充都会让它灭；召唤时另记一份「召唤后的面板指纹」，所以面板里本来有别的词时，
  刚召唤完也还是亮的。
...
- 门禁假红处理：`_ui_bridge.shot()` 五次重试全挂后**重建桥会话**再试一次 (L2 调试会话卡死只有
...
「我的」导出（13 分类 / 4753 标签 / 2.3MB）原样导回去 → HTTP 200，内容与 settings 一致。
门禁全量重跑见下。

## v1.11.0 — 预设重做：可见可选 + 记住当前面板的词标签（2026-09-21）
...
- **保存 = 当前面板词标签的快照**（`tags: [{en, zh, cat}]`）：旧实现只存"已经钉选的
  那几个词"，没钉就存不了 —— 用户报"记不住当前节点词标签"就是这个。现在没词才拒绝保存，
  排除域 / 配置照旧一起存。
- **召唤 = 快照里的词都打钉**：写进面板并 `pinned: true` → 自动模式（🎲 填充）不再覆盖
  这些词。出厂「场景预设」保持原语义（只有 `pinned`，约束不锁死、🎲 仍在框内随机），
  两类预设共用一个下拉，靠 `kind` 与是否有 `tags` 区分。
  真机验收：填充 53 词 → 保存 → 清空 → 召唤 → **53 词回位且均为钉选**；
...
## v1.9.1 — 自动模式若干口径调整（配额真源 / 眼色互斥 / 单人锁，2026-09-21）
...
## v1.7.1 — 输出文本若干调整（2026-09-12）

调整了若干提示词相关逻辑：未成年年龄词与成人内容的组合做了限制；多人场景的自然语言描述人称做了调整。
...
| **真机节点**（ComfyUI HTTP queue） | 只有 **约 30 次** —— `real_http_test` 是 7 个固定用例 + 1 次复现 + 20 个 seed 的**性别锁**，**没做文本层断言** |
...
| **默认关闭** | 新节点的 `exclude_categories` 默认含 `"画师"`，自动抽取时跳过它 |
...
`m2` 的束相邻、`perf_build`、`folder_template` 等行为门禁在重构后保持通过。
...
- **⚔ 武器档案**：档案 id / 中文名 / 挂载槽位 / 身份词（chip 可增删）都可改；
...
1. **「人数」槽混了 6 个非人数词**（`faceless` / `faceless female` / `faceless male` /
   ...
   被它们占掉就没有 `1girl`/`1boy` 出场，性别锁不置位。已归位到「构图镜头/构图」。
...
同时质量/细节层整层缺失：`score_*` 为 0 词、`ultra-detailed` / `huge filesize` /
...
- 第二轮 25 词：都取自群内那条提示词（即在用的词），含颜色+物体复合词
...
⚠ 每条标签必须有 `id`：`deep_merge` 是按 id 归并的，无 id 的标签会塌成同一个
...
- **修掉 `total_max` 的连带陷阱**：原实现是朴素截断（`(keep+rest)[:tmax]`），
  ...
  改为 `_trim_to_budget()`：按槽位贡献数从多到少削，并尽量不低于各槽位配额下限。
...
- **Fast/Smart 引擎切换**：`settings.random_configs` 根本不存在，
  实测 `resolve_config(default_fast)` 与 `resolve_config(default_smart)` 返回值相同
...
**菜单交互（实现细节，值得记一笔）**
- 关闭时机收在**面板作用域内**（面板内 pointerdown / 再点 ⋯ / Esc），
...
- **出厂库缺 `axis` 字段**：`data/default/tag_library.json` 的 4356 个标签都没有
  `axis`，而用户库与两份备份都有。引擎侧因 `t.get("axis") or axis_of(...)` 有兜底
  不受影响，但挑选器「🎯 拼装轴」视图用 `t.axis || "misc"`——一旦用户库被清空后
  由 md 模板重建，词会塌进「📦 未归类」。现改为在 `schema.migrate_tag()` 里按
...
- 挑选器 170 行内联硬编码色值都改为消费变量；管理页补齐浅色主题（原先零适配），
...
- **武器·物品档案（⚔ bundle）**：7 武器 + 11 日常物品 = 18 份档案、27 束成员词、93 行姿势表。每条姿势=标签组+手数+视线+状态槽+排斥声明；武器通常带持握姿势出生、姿势跟随武器出生——裸武器/双持单手刀/弓蹭枪姿势很少出现（万 seed 压测里束出生率接近满值）
...
**测试**
- 9 套 Python 门禁通过；万抽长跑未见违规；真机 ComfyUI HTTP queue 通过；CDP 浏览器逐 tab 截图巡检
