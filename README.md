...
### v1.2.2
- **反冲突压测优化**：300 次生成压测（无过滤 / ♂ / ♀ 各 100，12~30 词大抽取）驱动规则 55 → 81 条、零失效：
  服装套数互斥（裙装/职业/传统/特色 内部单选 + 两两互斥）、姿势基态单选、鞋/下装/袜/内裤/文胸单选、
  ahegao ↔ 微笑系表情矛盾、幼儿安全规则（幼儿 ↔ 成人体型/妆容/裸露/NSFW 状态词）、
  no humans ↔ 人物描述维度、降雨 ↔ 降雪、晴空 ↔ 降水；既有互斥组全面纳入补充词库新词
- **同义重复清理**：judo uniform 与 judo gi 去重
- **数据同步**：用户库（4356 标签）同步至默认库与出厂/用户备份，恢复往返字节级一致
