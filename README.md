...
- **🎲 分轴重摇**：填充分组标题上自带 🎲，只重摇该轴的词，其余轴都保留——「重摇→钉住满意的→再摇下一轴」的高效循环
...
- 低频词审计：`tools/danbooru_audit.py`（--verify 25 抽样线上校准，报告出「自造词/低频词/高频覆盖率」，实测出厂词表 ~55% 不在 danbooru 高频 35k 词表内）。逐轴处理与对照数据见 [docs/TAG-AND-OUTPUT-REVIEW-2026-09-23.md](docs/TAG-AND-OUTPUT-REVIEW-2026-09-23.md)
...
一键跑门禁（**推荐**，会自动快照并还原 `data/default/taglib/`，不会污染工作区）：
