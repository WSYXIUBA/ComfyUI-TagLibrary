...
### v1.3.0（本次，底层重构）
- **拼装轴**：4356 词按 12 轴重新聚合，树降级为浏览皮肤；挑标签面板双视图切换
- **武器·物品档案**：7 武器 + 11 物品 18 份档案、27 束成员词、93 行姿势表；武器必带姿势出生，束 all-or-nothing；档案可视化 + JSON 编辑
- **冲突结构化**：同轴单选/资源预算(hands≤2,gaze≤1)/状态槽/50 全局互斥域自动推导，跨池规则保留 `conflicts.json`；性别锁池级+出口级双保险
- **NL 尾段**：34 组句式族查表编译（人称回指/句式轮换/叙事顺序防拼接感），seed 可复现，可关闭；物品姿势句全量 Danbooru 真实 tag 验证，零生造词
- **UI 全面翻新**：8 页签挑选器（轴视图、⚔/🧬/✍ 三个新页、双语「文A」切换）、🎲 填充改走服务端真引擎（预览=实出）、设置新增 1.3.0 引擎区（NL 开关/带束概率/武器上限/配件概率）
- **引擎合一**：Fast/Smart 双引擎合并为单一结构引擎，旧面板引擎切换移除；`random_engine.py`/`rules_engine.py` 退役
- **重度测试**：9 套 Python 门禁全绿 + 万抽长跑零违规 + 真机 HTTP queue 全过 + CDP UI 巡检截图验收；抓到并修复测试子串误伤（`witch's hut` ≠ `witch`）、书+手机视线互斥缺口（63→0）
- 兼容：库格式只增字段不改语义，旧工作流直接加载；`selection_state` 协议不变

### v1.2.2
- **反冲突压测优化**：300 次生成压测驱动规则 55 → 81 条、零失效；幼儿安全规则、同义重复清理、数据同步

### v1.2.0
- **标签库 v2 结构重组**：9 大类 / 63 子分类 / 1615 词条；补充词库合并 +2700；出图元数据；翻译扩展免疫；性别过滤全链路一致；自动回显保留手动标签
