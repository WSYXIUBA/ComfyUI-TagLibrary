...
| 项 | 结果 |
|---|---|
| §2.1 读路径会写盘（P0） | ✅ 1.8.2 把热同步移出读路径；1.12.0 连同镜像层一起删掉，读路径彻底不写盘 |
| §2.2 `.md` 镜像往返产生畸形标签（P0） | 🗑 随镜像层删除而消失（1.12.0：整库只剩 .json） |
| §3.1 原子写抽公共工具 | ✅ `jsonio.atomic_write_json` |
| §3.2 `api/_common.py` 去业务依赖 | ✅ 并加导入白名单门禁 |
| §3.3 加 lint 门禁 | ✅ 死代码 + `_common` 白名单 +（1.12.2）版本号一致性 |
| §3.4 两个超大模块 | ✅ `tagfiles.py` 四拆（1.8.x）→ 1.12.0 整层删除；`engine.run_auto` 闭包提成 `_Extraction` 类 |
| §4.1 前端三个巨型文件 | ⏳ 未动，仍是单文件 100KB+ 量级（拆分方案见上） |
| §4.2 工作区遗留物 | ✅ 旧发布包早已清（1.8.x）；`tests/_scratch/` 51 个归档脚本与 1 个无引用迁移工具已删（1.12.2） |
| §4.3 前端残留测试钩子 | ✅ `window.__taglib` 已删（唯一使用者 `_scratch` 已清理） |
| §5「不建议动的地方」 | 除「规则真源外置的理由」外仍全部适用（镜像机制已不存在，该条理由作废，结论仍成立） |
